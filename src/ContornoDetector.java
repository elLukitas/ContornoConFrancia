...
public class ContornoDetector {
    ...
    public void detectarContorno(boolean mejorador, int nivel, int threshold, String nombre) {
        File grey;
        if (mejorador) {

            grey = blureador(nivel, true);
        } else {
            grey = blureador(nivel, false);
        }
...
//                    }
            ...
            File fileFiltrato = new File(nombre + ".jpg");
    ...
    public File blureador(int nivel, boolean mejorador) {
        File grey;
        if (nivel == 1) {
            return file;
        } else if (mejorador) {
    ...
    public File prisma(int level) {
        try {
            ...
            for (int i = 0; i < img.getWidth(); i += divisor) {
                ...
                for (int j = 0; j < img.getHeight(); j += divisor) {
            ...
            File fileModified = new File("prisma.jpg");
            ImageIO.write(contornoPuro, "jpg", fileModified);
            return fileModified;
